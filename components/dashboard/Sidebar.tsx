...
/* ------------------------------------------------------
   ✅ FIXED: SidebarContent moved OUTSIDE Sidebar()
-------------------------------------------------------*/
...
/* ------------------------------------------------------
   MAIN SIDEBAR COMPONENT (unchanged except call)
-------------------------------------------------------*/
...
export function Sidebar({ course, completedLessons = [] }: SidebarProps) {
  ...
  useEffect(() => {
    if (pathname && course?.modules) {
      ...
      if (
        currentModuleId &&
        !openModules.includes(currentModuleId)
      ) {
        // Compute the next openModules array
        const nextOpenModules = [...openModules, currentModuleId];
        // Only update if the array has changed
        setOpenModules(nextOpenModules);
      }
    }
  ...
  if (!course || !isMounted) return null;
